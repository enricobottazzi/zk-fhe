    ...
    let callback =
        move |ctx_gate: &mut Context<F>, ctx_rlc: &mut Context<F>, eth_chip: &EthChip<F>| {
            ...
            */
            ...
            // * the coefficients of quotient are constrained to be in the range [0, Q - 1] -> met according to the constraint just set above
            // * the coefficients of remainder are constrained to be in the range [0, Q - 1] -> met according to the constraint just set above
            ...
            // pk0_u is the reduction of pk0_u by the cyclotomic polynomial
            // pk0_u is a polynomial of degree N - 1
            ...
            // Perform the polynomial addition between pk0_u and m_delta.
            ...
            // `pk0_u` has coefficients in the [0, Q-1] range according to the constraint set above
            // The coefficients of pk0_u_plus_m_delta are in the [0, (Q-1) * (Q-T) + (Q-1)] range. Why? Answer is here -> https://hackmd.io/@letargicus/Bk4KtYkSp - Polynomial Addition section
            ...
            // pk0_u_plus_m_delta is a polynomial in the R_q ring

            // 1.6 c0 = pk0_u_plus_m_delta + e0

            // Perform the polynomial addition between pk0_u_plus_m_delta and e0.
            // The assumption of the `poly_add` chip is that the coefficients of the input polynomials are constrained such to avoid overflow during the polynomial addition
            // `pk0_u_plus_m_delta` has coefficients in the [0, (Q-1) * (Q-T) + (Q-1)] range according to the constraint set above
            // `e0` has coefficients in the [0, B] OR [Q-B, Q-1] range according to the constraint set above
            // The coefficients of c0 are in the [0, (Q-1) * (Q-T) + (Q-1) + (Q-1)] range. Why? Answer is here -> https://hackmd.io/@letargicus/Bk4KtYkSp - Polynomial Addition section
